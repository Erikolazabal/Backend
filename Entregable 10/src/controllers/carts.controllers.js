...
class CartsController {
  ...
  updateProductQuantity = async (req, res, next) => {
    ...
    const { prodId, cartId } = req.params;
    try {
      const updatedCart = await cartsService.updateProductQuantityById(
        cartId,
        prodId,
        quantity
      );
  ...
  createCart = async (req, res, next) => {
    try {
      ...
      res.send(cart);
      logger.debug('Carrito creado con éxito');
    } catch (error) {
      next(error);
    }
  };
    ...
    try {
      ...
      for (let i = 0; i < cartData.products.length; i++) {
        ...
        if (product.stock >= cartData.products[i].quantity) {
          ...
          await productsService.updateProductById(product._id, {
      ...
      const code = tickets.length > 0 ? parseInt(tickets[tickets.length - 1].code) + 1 : 1;
